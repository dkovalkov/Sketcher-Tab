...
public final class SurfaceDiff {
    ...
    public static native void findBounds(Bitmap original, Bitmap updatedSurf, int originalWidth, int originalHeight, byte[] bitmask, int[] bounds, int[] pixels);
...
//        Rect diffBounds = new Rect(originalWidth + 1, originalHeight + 1, -1, -1);
        ...
        findBounds(original, updatedSurf, originalWidth, originalHeight, bitmask, nativeBounds, pixels);

        Rect diffBounds = new Rect(nativeBounds[0], nativeBounds[2], nativeBounds[1], nativeBounds[3]);
...
//        }
        ...
        boolean[] bMask = new boolean[bitmask.length];
        for (int i = 0; i < bitmask.length; i += 1) {
            bMask[i] = bitmask[i] == 1;
        }
        return new SurfaceDiff(bMask, diffBounds, pixels);
    }
